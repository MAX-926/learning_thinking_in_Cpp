...
inline void requireArgs(int argc, int args, const std::string& msg = "Must use %d arguments")
{
    ...
    if(argc != args + 1)
    {
        std::fprintf(stderr, msg.c_str(), args);
        fputs("\n", stderr);
        exit(1);
    }
}

inline void requireMinArgs(int argc, int minArgs, const std::string& msg = "Must use at least %d argumments")
{
    using namespace std;
    if(argc < minArgs + 1)
    {
...
inline void assure(std::ifstream& in, const std::string& filename = "")
{
    ...
    if(!in)
    {
        fprintf(stderr, "Could not open file &s\n", filename.c_str());//string.c_str()将string转换成const char*类型
        exit(1);
    }
}
